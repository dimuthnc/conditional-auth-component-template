...
import org.wso2.carbon.identity.extensions.authenticator.conditional.auth.functions.function.GetDataFunctionImpl;

@Component(
        name = "session.based.conditional.authentication.function.component",
        immediate = true
)
public class JSFunctionsComponent {

    private static Log log = LogFactory.getLog(JSFunctionsComponent.class);

    private JsFunctionRegistry jsFunctionRegistry;
    private GetDataFunctionImpl getDataFunctionImpl;

    @Activate
    protected void activate(ComponentContext ctxt) {

        try {
            getDataFunctionImpl = new GetDataFunctionImpl();
            jsFunctionRegistry.register(JsFunctionRegistry.Subsystem.SEQUENCE_HANDLER, "getSessionData",
                    (GetDataFunction) getDataFunctionImpl::getData);
